...
func IsValidEmail(ctx icontext.Ctx, str string) error {
	re := regexp.MustCompile("[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@" +
		"(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")
	res := re.MatchString(str)
	if !res {
		return errors.New(ctx.T("Invalid e-mail address."))
	}

	return nil
}

func IsValidPassword(ctx icontext.Ctx, str string) error {
	if len(str) < 6 {
		return errors.New(ctx.T("Password is too short. It must contain at least 6 characters."))
	}

	re := regexp.MustCompile("[a-z0-9!#$%&]+")
	res := re.MatchString(str)
	if !res {
		return errors.New(ctx.T("Invalid character in password. Allowed characters:|| 'a-z0-9!#$%&'"))
	}
