...
func Insert(rw http.ResponseWriter, req *http.Request) {
	...
	_, err := help_lib.GetHelpRec(helpId)
	if err != nil {
	...
	sqlStr := `insert into
					helpImage(helpImageId, helpId,
					imgType, imgName, imgMime, imgSize, imgHeight, imgWidth, imgData)
					values(null, ?, ?, ?, ?, ?, ?, ?, ?)`

	_, err = tx.Exec(sqlStr, helpId, "alternative", imgName, imgMime, imgSize, imgConfig.Height,
...
func insertForm(ctx *context.Ctx) {
	...
	urlStr := ctx.U("/help_image_insert", "helpId", "key", "pn")
