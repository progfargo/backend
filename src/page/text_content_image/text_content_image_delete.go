...
func Delete(rw http.ResponseWriter, req *http.Request) {
	...
	if n, _ := res.RowsAffected(); n == 0 {
		tx.Rollback()
		ctx.Msg.Warning(ctx.T("Record could not be found."))
		ctx.Redirect(ctx.U("/text_content_image", "imgId", "key", "pn"))
		return
	}
...
func deleteConfirm(ctx *context.Ctx, rec *text_content_image_lib.TextContentImageRec) {
	buf := util.NewBuf()
	...
	buf.Add(content.BackButton(ctx, ctx.U("/text_content_image", "textId", "key", "pn")))
	buf.Add("</div>")
	buf.Add("</div>")
	...
	imgUrl := ctx.U("/text_content_image_middle", "imgId")
	...
	urlStr := ctx.U("/text_content_image_delete", "textId", "imgId", "confirm", "key", "pn", "stat")
