...
func Insert(rw http.ResponseWriter, req *http.Request) {
	...
	sqlStr := `insert into
					newsImage(newsImageId, newsId,
					imgType, imgName, imgMime, imgSize, imgHeight, imgWidth, imgData)
					values(null, ?, ?, ?, ?, ?, ?, ?, ?)`

	_, err = tx.Exec(sqlStr, newsId, "alternative", imgName, imgMime, imgSize, imgConfig.Height,
...
func insertForm(ctx *context.Ctx) {
	...
	urlStr := ctx.U("/news_image_insert", "newsId", "key", "pn", "stat")
