...
func UpdateImage(rw http.ResponseWriter, req *http.Request) {
	...
	newsRec, err := news_lib.GetNewsRec(newsId)
	if err != nil {
	...
	if ctx.Req.Method == "GET" {
		updateImageForm(ctx, newsRec, imgRec)
		return
	}
...
func updateImageForm(ctx *context.Ctx, newsRec *news_lib.NewsRec, imgRec *news_image_lib.NewsImageRec) {
	buf := util.NewBuf()
	...
	urlStr = ctx.U("/news_image_update", "newsId", "key", "pn", "stat", "imgId")
