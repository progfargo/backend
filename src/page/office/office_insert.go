...
func Insert(rw http.ResponseWriter, req *http.Request) {
	...
	sqlStr := `insert into
					office(officeId, name, city, address, telephone, email, mapLink,
					imgName, imgMime, imgSize, imgData)
					values(null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`

	_, err = tx.Exec(sqlStr, name, city, address, telephone, email, mapLink,
		email, "image/png", emailBuf.Len(), emailBuf.Bytes())
	if err != nil {
		tx.Rollback()
...
func insertForm(ctx *context.Ctx) {
	...
	buf.Add(content.PageTitle(ctx.T("Offices"), ctx.T("New Record")))
	buf.Add("</div>")
