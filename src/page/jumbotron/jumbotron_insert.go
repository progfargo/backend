...
func Insert(rw http.ResponseWriter, req *http.Request) {
	...
	if !ctx.IsRight("jumbotron", "insert") {
		app.BadRequest()
	}
	...
	sqlStr := `insert into
					jumbotron(jumbotronId, title, body)
					values(null, ?, ?)`

	_, err = tx.Exec(sqlStr, title, body)
	if err != nil {
		tx.Rollback()
...
func insertForm(ctx *context.Ctx) {
	...
	ctx.Js.Add("/asset/js/page/jumbotron/jumbotron_insert.js")
	...
	buf.Add(content.PageTitle(ctx.T("Jumbotron"), ctx.T("New Record")))
	buf.Add("</div>")
