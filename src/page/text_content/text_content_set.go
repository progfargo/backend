...
func Set(rw http.ResponseWriter, req *http.Request) {
	...
	if !ctx.IsRight("text_content", "set") {
		app.BadRequest()
	}
	...
	res, err := tx.Exec(sqlStr, exp, body, textId)
	if err != nil {
		tx.Rollback()
	...
	ctx.Redirect(ctx.U("/text_content_set", "textId"))
}

func setForm(ctx *context.Ctx, rec *text_content_lib.TextContentRec) {
	content.Include(ctx)

	ctx.Js.Add("/asset/tinymce/tinymce.min.js")
	ctx.Js.Add("/asset/tinymce/tinymce_func.js")
	ctx.Js.Add("/asset/js/page/text_content/text_content_set.js")
	...
	buf.Add(content.PageTitle(ctx.T("Text Content"), ctx.T("Set Record")))
	buf.Add("</div>")
	...
	textMenu.Set(ctx, "text_content_set")
	...
	buf.Add("<input type=\"text\" name=\"name\" class=\"formControl\""+
		" value=\"%s\" maxlength=\"100\" tabindex=\"1\" disabled>", util.ScrStr(rec.Name))
	buf.Add("</div>")
